...
public class WebServer {
  ...
  /**
   * The URL pattern to go to after you are logged in.
   */
  public static final String LOGGED_IN_URL = "/home";

  /**
   * The URL pattern to go to in order to sign out.
   */
  ...
  public void initialize() {
    ...
    // The player lobby, stores the players that are playing the game
    PlayerLobby lobby = new PlayerLobby();

    // Shows the Checkers game Home page.
    get(HOME_URL, new GetHomeRoute(lobby, templateEngine));

    // Shows the Checkers sign in page.
    get(SIGNIN_URL, new GetSignInRoute(templateEngine));

    // Handle the webpages for when the user is logging into the server.
    post(SIGNIN_URL, new PostSignInRoute(lobby, templateEngine));
    get(LOGGED_IN_URL, new GetHomeRoute(lobby, templateEngine));

    // Handle the starting of a game
    get(GAME_URL, new GetGameRoute(lobby, templateEngine));

    // When the user signs out of the game, log them out of the server.
    post(SIGNOUT_URL, new PostSignOutRoute(lobby, templateEngine));

    post(VALIDATE_MOVE_URL, new PostValidateMoveRoute(lobby, templateEngine));

    // Log displays that the server is initialized.
